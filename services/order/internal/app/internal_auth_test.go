...
func TestSignedInternalMetadataRoundTrip(t *testing.T) {
	...
	ctx := internalcall.AppendUser(context.Background(), secret, "/order/test", 9, now)
	values, ok := metadata.FromOutgoingContext(ctx)
	if !ok {
		t.Fatal("signed metadata missing")
	}
	if values.Get(internalcall.SignatureKey)[0] == "" {
	...
	ctx = internalcall.AppendUser(context.Background(), secret, "/order/test", 9, time.Now())
...
func TestSignedRoleMetadataRejectsCustomerForAdminCall(t *testing.T) {
	secret := "internal-secret"
	t.Setenv("SECKILL_INTERNAL_CALL_SECRET", secret)
	ctx := internalcall.AppendRole(context.Background(), secret, "/order/ship", 9, "customer", time.Now())
	ctx = metadata.NewIncomingContext(context.Background(), outgoingToIncoming(ctx))
	...
	ctx = internalcall.AppendRole(context.Background(), secret, "/order/ship", 9, "admin", time.Now())
	ctx = metadata.NewIncomingContext(context.Background(), outgoingToIncoming(ctx))
	if err := AuthorizeInternalRoleRequest(ctx, "/order/ship", 9, "admin"); err != nil {
...
func TestExpireRequiresSystemRoleWhenInternalAuthEnabled(t *testing.T) {
	...
	t.Setenv("SECKILL_INTERNAL_CALL_SECRET", secret)
	method := "/order/expire"
	ctx := internalcall.AppendRole(context.Background(), secret, method, systemActorID, "admin", time.Now())
	ctx = metadata.NewIncomingContext(context.Background(), outgoingToIncoming(ctx))
	...
	ctx = internalcall.AppendRole(context.Background(), secret, method, systemActorID, systemRole, time.Now())
	ctx = metadata.NewIncomingContext(context.Background(), outgoingToIncoming(ctx))
	if err := AuthorizeInternalRoleRequest(ctx, method, systemActorID, systemRole); err != nil {
