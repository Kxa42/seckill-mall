...
func registerOrderRoutes(router *gin.Engine, client pb.CommerceOrderServiceClient, cartClients ...pb.CartServiceClient) {
	...
	authenticated.GET("/orders", func(c *gin.Context) {
		...
		defer cancel()
		ctx = internalcall.AppendUser(ctx, os.Getenv("SECKILL_INTERNAL_CALL_SECRET"), pb.CommerceOrderService_List_FullMethodName, userID, time.Now())
	...
	authenticated.GET("/orders/:order_id", func(c *gin.Context) {
		...
		ctx, cancel := context.WithTimeout(c.Request.Context(), orderRPCTimeout)
		defer cancel()
		ctx = internalcall.AppendUser(ctx, os.Getenv("SECKILL_INTERNAL_CALL_SECRET"), pb.CommerceOrderService_Get_FullMethodName, userID, time.Now())
	...
	authenticated.POST("/orders/:order_id/cancel", func(c *gin.Context) {
		...
		ctx, cancel := context.WithTimeout(c.Request.Context(), orderRPCTimeout)
		defer cancel()
		ctx = internalcall.AppendUser(ctx, os.Getenv("SECKILL_INTERNAL_CALL_SECRET"), pb.CommerceOrderService_Cancel_FullMethodName, userID, time.Now())
...
func listCartForOrder(c *gin.Context, client pb.CartServiceClient, userID uint64) []orderItemRequest {
	ctx, cancel := context.WithTimeout(c.Request.Context(), orderRPCTimeout)
	defer cancel()
	ctx = internalcall.AppendUser(ctx, os.Getenv("SECKILL_INTERNAL_CALL_SECRET"), pb.CartService_List_FullMethodName, userID, time.Now())
...
func createOrderRPC(c *gin.Context, client pb.CommerceOrderServiceClient, request *pb.OrderCreateRequest) *pb.OrderCreateResponse {
	ctx, cancel := context.WithTimeout(c.Request.Context(), orderRPCTimeout)
	defer cancel()
	ctx = internalcall.AppendUser(ctx, os.Getenv("SECKILL_INTERNAL_CALL_SECRET"), pb.CommerceOrderService_Create_FullMethodName, request.GetUserId(), time.Now())
